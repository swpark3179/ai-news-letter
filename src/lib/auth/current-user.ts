...
export async function upsertMemberFromSso(decoded: UpsertInput): Promise<SessionUser> {
  ...
  if (existing) {
    ...
    if (decoded.epid && decoded.epid !== existing.epid) patch.epid = decoded.epid;
...
export async function resolveMemberFromSso(decoded: DecodedUser): Promise<SessionUser> {
  ...
  let row = await findMemberBy(db, "epid", decoded.epid);
  if (!row) row = await findMemberBy(db, "emp_no", decoded.empNo);
  ...
  const patch: Partial<MemberRow> = {};
  if (!row.epid && decoded.epid) patch.epid = decoded.epid; // 최초 SSO 로그인 백필
